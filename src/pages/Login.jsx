  ...
  return (
    <div className="sm:flex">
      ...
      <div className="flex flex-col w-full p-4 sm:w-1/2 md:ml-5 lg:ml-16 md:ml-36 sm:pt-44 lg:pr-44">
        <h1 className="text-5xl font-semibold text-center sm:text-left text-[#1B345F] mb-3 ">Hello Again!</h1>
        ...
        <form className="flex flex-col w-full">
          ...
          <input className="w-full h-14 border-2 border-[#F0F4FD] rounded-md pl-3 mb-6" type="email" onChange={handleEmail} placeholder="Email adress" />

          <label className="text-lg mb-3">Password</label>
          <input className="w-full h-14 border-2 border-[#F0F4FD] rounded-md pl-3 mb-4" placeholder="Password" type="password" onChange={handlePassword} />

          <p className="text-lg text-[#F7731C] text-right mb-10">Reset Password</p>

          <button className="text-lg font-medium text-[#1B345F] w-full bg-[#DBE5FA] h-14 rounded-md" onClick={() => handleRegister()}>Create New Account</button>
        </form>
        <button className="text-lg font-medium text-white w-full bg-[#F7731C] h-14 rounded-md mt-5" onClick={() => handleLogin()}>Sign In</button>
      </div>
    </div>
  );
};
